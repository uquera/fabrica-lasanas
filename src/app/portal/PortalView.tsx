...
import { LogOut, Package, ChevronDown, Eye, Search, X } from "lucide-react";
...
export default function PortalView({ envios, razonSocial }: { envios: Envio[]; razonSocial: string }) {
  ...
  // Filtrar por búsqueda (sucursal o folio)
  ...
  // Totales del período completo (no afectados por búsqueda)
  const totales = useMemo(() => {
    let unidades = 0;
    let neto = 0;
    for (const envio of enviosPeriodo) {
    ...
    return { unidades, neto, iva, total };
  }, [enviosPeriodo]);

  // Totales de la búsqueda activa
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div>
            ...
            <div className="relative">
              <select
                value={mes}
                onChange={(e) => { setMes(Number(e.target.value)); setBusqueda(""); }}
          ...
          <div>
            ...
            <div className="relative">
              <select
                value={año}
                onChange={(e) => { setAño(Number(e.target.value)); setBusqueda(""); }}
                className="appearance-none bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 pr-8 text-sm focus:outline-none focus:border-orange-500/50"
              >
        ...
        {/* Resumen cards — siempre del período completo */}
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-sm">
                Detalle de guías — {MESES[mes]} {año}
              </h2>
            </div>
            {hayBusqueda && (
              <span className="text-xs text-zinc-500">
                {enviosFiltrados.length} de {enviosPeriodo.length} resultados
              </span>
            )}
          </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {enviosFiltrados.map((envio) => {
                    ...
                    const totalEnvio = netoEnvio * 1.19;
                    ...
                    return (
                      <tr key={envio.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
                        ...
                        <td className="px-5 py-3.5 text-right text-zinc-300">{fmt(netoEnvio)}</td>
                ...
                <tfoot>
                  <tr className="bg-zinc-800/30 font-bold">
                    ...
                    <td className="px-5 py-3.5 text-right">{hayBusqueda ? fmt(totalesFiltrados.neto) : fmt(totales.neto)}</td>
